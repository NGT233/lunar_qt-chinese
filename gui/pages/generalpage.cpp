...
GeneralPage::GeneralPage(Config &config, QWidget *parent) : ConfigurationPage(config, parent) {
    ...
    connect(initialMemory, &QSlider::valueChanged,
            [initialMemoryLabel](int val) { initialMemoryLabel->setText("初始内存： " + QString::number(val) + " MiB"); });
    connect(maxMemory, &QSlider::valueChanged,
            [maxMemoryLabel](int val) { maxMemoryLabel->setText("最大内存:  " + QString::number(val) + " MiB"); });
    ...
    useCustomJre = new QCheckBox(QStringLiteral("使用自定义JRE"));
    ...
    jvmArgsGroup->addWidget(new QLabel(QStringLiteral("JVM参数")), 0, Qt::AlignHCenter);
